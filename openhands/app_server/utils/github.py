...
Requires env vars: GITHUB_APP_ID + GITHUB_APP_PRIVATE_KEY.
"""
...
def _resolve_token(repository: str) -> str:
    ...
    Raises:
        RuntimeError: If GitHub App is not configured or token
            resolution fails.
    """
    ...
    return GitHubAppTokenManager.get_token_for_repository(owner, repo_name)
...
def add_pr_comment(repository: str, pr_number: int, body: str) -> dict:
    ...
    Raises:
        RuntimeError: If GitHub App is not configured or the API
            call fails after retry.
    """
    ...
    try:
        ...
        if status in (401, 403):
            ...
            owner, _, repo_name = repository.partition('/')
            try:
                fresh_token = GitHubAppTokenManager.refresh_token(
                    owner, repo_name
                )
