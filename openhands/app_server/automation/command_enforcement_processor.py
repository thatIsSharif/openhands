...
class CommandEnforcementProcessor(EventCallbackProcessor):
    ...
    async def __call__(
        ...
        _logger.warning(
            '[Security] Layer 2 enforcement triggered for conversation %s: '
            'pattern=%s command=%r',
            conversation_id,
            matched_label,
            command[:500],
        ...
        await self._post_rejection_comment(record, command)

        # Interrupt the running conversation
        await self._interrupt_conversation(conversation_id)
    ...
    async def _post_rejection_comment(self, record, command: str) -> None:
    ...
    async def _interrupt_conversation(self, conversation_id: UUID) -> None:
